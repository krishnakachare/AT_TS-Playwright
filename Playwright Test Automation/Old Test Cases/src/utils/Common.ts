...
import AzureDevOps from "./AzureDevOpsHelper";
import * as fs from "fs";
import { TestReport } from "../interface/TestResults.interface";
...
export async function waitUntilAppIdle(page: Page): Promise<void> {
  try {
    ...
    console.log(
      `waitUntilIdle failed, ignoring.., error: ${(e as Error).message}`,
    );
  }
}
...
async function readJsonReport() {
  ...
  if (
    process.env.UPDATE_TEST_PLAN === "Yes" &&
    process.env.PIPELINE === "Yes"
  ) {
          ...
          spec.tests.forEach((test) => {
            test.results.forEach(async (result) => {
              ...
              for (const testCaseId of numbersArray) {
                console.log(
                  `Test Case & Status : ${testCaseId} : ${testCaseStatus}`,
                );
                await azureDevOps.updateTestCaseStatus(
                  String(testCaseId),
                  testCaseStatus,
                );
              }
...
export async function loginToApplication(page: Page) {
  ...
  await expect(page.locator("#welcomeMsg")).toContainText(
    "Welcome to Testers Talk!",
  );
}
